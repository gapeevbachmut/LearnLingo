...
${e.levels.map(n=>`<li class='teacher-level'><p>#${n}</p></li>`).join("")}
    ...
    `}const o=document.querySelector(".teachers-list"),f=document.querySelector(".load-more-btn"),_=4;let m=1,i=[];function c(e){i=e,m=1,L()}function L(){const e=m*_,s=i.slice(0,e),n=s.map((t,a)=>k(t,a)).join("");o.innerHTML=n,s.length>=i.length&&(f.style.display="none")}f.addEventListener("click",()=>{m+=1,L()});o.addEventListener("click",e=>{const s=e.target.closest(".teacher-read-more");if(!s)return;const n=s.closest(".teacher-card"),t=n.querySelector(".teacher-extra"),a=n.querySelector(".trial-lesson-btn");o.querySelectorAll(".teacher-card").forEach(r=>{r!==n&&(r.querySelector(".teacher-extra")?.classList.add("is-hidden"),r.querySelector(".trial-lesson-btn")?.classList.add("is-hidden"),r.querySelector(".teacher-read-more")?.classList.remove("is-hidden"))}),t.classList.toggle("is-hidden")?(s.classList.remove("is-hidden"),a.classList.add("is-hidden")):(s.classList.add("is-hidden"),a.classList.remove("is-hidden"));const $=Number(n.dataset.index),b=i[$],y=r=>{r.stopPropagation(),S({type:"trial",teacher:b})};a&&a.addEventListener("click",y)});const l=document.querySelector(".scroll-top"),w=()=>{window.scrollY>350?l.classList.remove("is-hidden"):l.classList.add("is-hidden")};window.addEventListener("scroll",w);w();const P=()=>window.scrollTo({top:0,behavior:"smooth"});l.addEventListener("click",P);const d=document.querySelector(".language");let u=[];function M(e){u=e;const s=e.flatMap(t=>t.languages);[...new Set(s)].forEach(t=>{const a=document.createElement("option");a.value=t,a.textContent=t,d.append(a)}),d.value=""}d.addEventListener("change",e=>{const s=e.target.value;if(!s){c(u);return}const n=u.filter(t=>t.languages.includes(s));c(n)});const j="https://learnlingo-12-25-default-rtdb.firebaseio.com/.json";async function R(){const e=await E.get(j);return e.data?Object.values(e.data):[]}const p=document.querySelector(".level");let h=[];function B(e){h=e;const s=e.flatMap(t=>t.levels);[...new Set(s)].forEach(t=>{const a=document.createElement("option");a.value=t,a.textContent=t,p.append(a)}),p.value=""}p.addEventListener("change",e=>{const s=e.target.value;if(!s){c(h);return}const n=h.filter(t=>t.levels.includes(s));c(n)});const v=document.querySelector(".price");let g=[];function F(e){g=e;const s=e.map(t=>t.price_per_hour);[...new Set(s)].sort((t,a)=>t-a).forEach(t=>{const a=document.createElement("option");a.value=t,a.textContent=`${t} $`,v.append(a)}),v.value=""}v.addEventListener("change",e=>{const s=e.target.value,n=Number(s);if(!s){c(g);return}const t=g.filter(a=>a.price_per_hour===n);c(t)});T();async function A(){const e=await R();c(e),M(e),B(e),F(e)}A();q();
