import{i as h,g as b,c as d,b as i,s as y,d as g,u as v,e as w,o as k}from"./vendor-CJFyyV97.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))a(o);new MutationObserver(o=>{for(const s of o)if(s.type==="childList")for(const n of s.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&a(n)}).observe(document,{childList:!0,subtree:!0});function r(o){const s={};return o.integrity&&(s.integrity=o.integrity),o.referrerPolicy&&(s.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?s.credentials="include":o.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function a(o){if(o.ep)return;o.ep=!0;const s=r(o);fetch(o.href,s)}})();const E={apiKey:"AIzaSyBM5Y6QM2r6WXTF8zM7dWPy11UsmUn6HlI",authDomain:"learnlingo-auth-320f9.firebaseapp.com",projectId:"learnlingo-auth-320f9",storageBucket:"learnlingo-auth-320f9.firebasestorage.app",messagingSenderId:"1060134815936",appId:"1:1060134815936:web:65e14830a23616ff3e0525"},x=h(E),c=b(x),M=d({username:i().required("Name is required").min(3,"Minimum 3 characters").max(30,"Max 30 characters"),email:i().required("Email is required").email("Enter a valid email, for example: user@gmail.com"),phoneNumber:i().required("Phone is required").matches(/^\+?\d{10,14}$/,"Phone format: +380XXXXXXXXX").transform(e=>e.replace(/\s|-/g,""))}),S=d({email:i().required("Email is required").email("Enter a valid email, for example: user@gmail.com"),password:i().required("Password is required").min(8,"Minimum 8 characters").max(30,"Max 30 characters")}),L=d({username:i().required("Name is required").min(3,"Minimum 3 characters").max(30,"Max 30 characters"),email:i().required("Email is required").email("Enter a valid email, for example: user@gmail.com"),password:i().required("Password is required").min(8,"Minimum 8 characters").max(30,"Max 30 characters")});function m(e){const t={};e.inner.forEach(r=>{t[r.path]=r.message}),Object.entries(t).forEach(([r,a])=>{const o=document.querySelector(`[name="${r}"]`);if(!o)return;let s=o.nextElementSibling;(!s||!s.classList.contains("error"))&&(s=document.createElement("p"),s.classList.add("error"),o.after(s)),s.textContent=a})}function q(e){const t=document.querySelector(".form-lesson");t&&t.addEventListener("submit",async r=>{r.preventDefault(),t.querySelectorAll(".error").forEach(s=>s.remove());const a=new FormData(t),o=Object.fromEntries(a.entries());try{await M.validate(o,{abortEarly:!1}),console.log("BOOKING-FORM DATA:",o),e()}catch(s){m(s)}})}async function A(e,t){return(await y(c,e,t)).user}function z(e){const t=document.querySelector(".form-login");t&&t.addEventListener("submit",async r=>{r.preventDefault(),t.querySelectorAll(".error").forEach(s=>s.remove());const a=new FormData(t),o=Object.fromEntries(a.entries());try{await S.validate(o,{abortEarly:!1}),console.log("LOGIN-FORM DATA:",o),await A(o.email,o.password),e()}catch(s){m(s)}})}function C(){return`
...
<button class="btn-register" type='submit'>Sign Up</button>
  ...
  `}async function P(e,t,r){const a=await g(c,t,r);return await v(a.user,{displayName:e}),a.user}function T(e){const t=document.querySelector(".form-register");t&&t.addEventListener("submit",async r=>{r.preventDefault(),t.querySelectorAll(".error").forEach(s=>s.remove());const a=new FormData(t),o=Object.fromEntries(a.entries());try{await L.validate(o,{abortEarly:!1}),console.log("REGISTER-FORM DATA:",o),await P(o.username,o.email,o.password),e()}catch(s){m(s)}})}const l=document.querySelector("#modal-root");function I(){document.body.classList.add("no-scroll")}function D(){document.body.classList.remove("no-scroll")}function p(e){const{type:t,teacher:r}=e;let a="";switch(t){case"trial":a=O(r);break;case"login":a=C();break;case"register":a=B();break;default:return}l.innerHTML=a,I();const o=l.querySelector(".backdrop"),s=l.querySelector(".closeButton"),n=()=>{l.innerHTML="",D(),document.removeEventListener("keydown",f)},f=u=>{u.key==="Escape"&&n()};o.addEventListener("click",u=>{u.target===o&&n()}),s.addEventListener("click",n),document.addEventListener("keydown",f),t==="trial"&&q(n),t==="login"&&z(n),t==="register"&&T(n)}function F(e){e.innerHTML=`
    <li>
  ...
  `,e.querySelector(".logIn").addEventListener("click",()=>p({type:"login"})),e.querySelector(".register").addEventListener("click",()=>p({type:"register"}))}async function H(){await w(c)}function N(e){return`
    <div class="out-box">
  ...
  <p class="out-text">Hello, ${e.displayName||e.email} </p>
    </div>`}function j(e,t){e.innerHTML=N(t),e.addEventListener("click",async r=>{r.target.classList.contains("logout")&&await H()})}function R(){const e=document.querySelector(".auth-list");e&&k(c,t=>{t?j(e,t):F(e)})}const X=new Date;document.querySelector(".footer-date");const U=document.querySelector(".date-year");U.textContent=X.getFullYear();const $=`<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style="position:absolute;width:0;height:0;overflow:hidden">
  <defs>
...
`;function W(){const e=document.createElement("svg");e.style.display="none",e.innerHTML=$,document.body.prepend(e)}function V(){document.addEventListener("click",e=>{const t=e.target.closest(".password-toggle");if(!t)return;const r=t.closest(".password-box");if(!r)return;const a=r.querySelector("input"),o=t.querySelector("use");if(!a||!o)return;const s=a.type==="password";a.type=s?"text":"password",o.setAttribute("href",s?"#icon-eye":"#icon-eye-off"),t.setAttribute("aria-label",s?"Hide password":"Show password")})}function Y(){const e=document.querySelector(".scroll-top");if(!e)return;const t=()=>{window.scrollY>350?e.classList.remove("is-hidden"):e.classList.add("is-hidden")};window.addEventListener("scroll",t),t();const r=()=>window.scrollTo({top:0,behavior:"smooth"});e.addEventListener("click",r)}R();W();V();Y();export{p as m};
//# sourceMappingURL=main-BO4joCab.js.map
