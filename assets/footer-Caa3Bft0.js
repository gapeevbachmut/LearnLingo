import{c as f,b as i,i as h,g as v,s as g,d as w,u as k,o as E}from"./vendor-D2CNYbfS.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))s(e);new MutationObserver(e=>{for(const o of e)if(o.type==="childList")for(const n of o.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&s(n)}).observe(document,{childList:!0,subtree:!0});function a(e){const o={};return e.integrity&&(o.integrity=e.integrity),e.referrerPolicy&&(o.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?o.credentials="include":e.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function s(e){if(e.ep)return;e.ep=!0;const o=a(e);fetch(e.href,o)}})();const L=`<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style="position:absolute;width:0;height:0;overflow:hidden">
  <defs>
...
`;function H(){const r=document.createElement("svg");r.style.display="none",r.innerHTML=L,document.body.prepend(r)}const S=f({username:i().required("Name is required").min(3,"Minimum 3 characters").max(30,"Max 30 characters"),email:i().required("Email is required").email("Invalid email"),phoneNumber:i().required("Phone is required").matches(/^\+?\d{10,14}$/,"Invalid phone number. +3801111111")}),M=f({email:i().required("Email is required").email("Invalid email"),password:i().required("Password is required").min(8,"Minimum 8 characters")}),x=f({username:i().required("Name is required").min(3,"Minimum 3 characters").max(30,"Max 30 characters"),email:i().required("Email is required").email("Invalid email"),password:i().required("Password is required").min(8,"Minimum 8 characters")});function p(r){const t={};r.inner.forEach(a=>{t[a.path]=a.message}),Object.entries(t).forEach(([a,s])=>{const e=document.querySelector(`[name="${a}"]`);if(!e)return;let o=e.nextElementSibling;(!o||!o.classList.contains("error"))&&(o=document.createElement("p"),o.classList.add("error"),e.after(o)),o.textContent=s})}function q(r){const t=document.querySelector(".form-lesson");t&&t.addEventListener("submit",async a=>{a.preventDefault(),t.querySelectorAll(".error").forEach(o=>o.remove());const s=new FormData(t),e=Object.fromEntries(s.entries());try{await S.validate(e,{abortEarly:!1}),console.log("BOOKING-FORM DATA:",e),r()}catch(o){p(o)}})}const C={apiKey:"***",authDomain:"***",projectId:"***",storageBucket:"***",messagingSenderId:"***",appId:"***"},A=h(C),y=v(A);async function z(r,t){return(await g(y,r,t)).user}function B(r){const t=document.querySelector(".form-login");t&&t.addEventListener("submit",async a=>{a.preventDefault(),t.querySelectorAll(".error").forEach(o=>o.remove());const s=new FormData(t),e=Object.fromEntries(s.entries());try{await M.validate(e,{abortEarly:!1}),console.log("LOGIN-FORM DATA:",e),await z(e.email,e.password),r()}catch(o){p(o)}})}function D(){return`
...
<button class="btn-register" type='submit'>Sign Up</button>
  ...
  `}async function P(r,t,a){const s=await w(y,t,a);return await k(s.user,{displayName:r}),s.user}function N(r){const t=document.querySelector(".form-register");t&&t.addEventListener("submit",async a=>{a.preventDefault(),t.querySelectorAll(".error").forEach(o=>o.remove());const s=new FormData(t),e=Object.fromEntries(s.entries());try{await x.validate(e,{abortEarly:!1}),console.log("REGISTER-FORM DATA:",e),await P(e.username,e.email,e.password),r()}catch(o){p(o)}})}const l=document.querySelector("#modal-root");function F(){document.body.classList.add("no-scroll")}function j(){document.body.classList.remove("no-scroll")}function c(r){const{type:t,teacher:a}=r;let s="";switch(t){case"trial":s=O(a);break;case"login":s=D();break;case"register":s=I();break;default:return}l.innerHTML=s,F();const e=l.querySelector(".backdrop"),o=l.querySelector(".closeButton"),n=()=>{l.innerHTML="",j(),document.removeEventListener("keydown",b)},b=m=>{m.key==="Escape"&&n()};e.addEventListener("click",m=>{m.target===e&&n()}),o.addEventListener("click",n),document.addEventListener("keydown",b),t==="trial"&&q(n),t==="login"&&B(n),t==="register"&&N(n)}const d=document.querySelector(".logIn"),u=document.querySelector(".register");d&&d.addEventListener("click",r=>{r.preventDefault(),c({type:"login"})});u&&u.addEventListener("click",r=>{r.preventDefault(),c({type:"register"})});E(y,r=>{r?(console.log("name",r.displayName),console.log("Logged in:",r.email)):(d&&d.addEventListener("click",t=>{t.preventDefault(),c({type:"login"})}),u&&u.addEventListener("click",t=>{t.preventDefault(),c({type:"register"})}),console.log("Not logged in"))});const T=new Date;document.querySelector(".footer-date");const R=document.querySelector(".date-year");R.textContent=T.getFullYear();export{H as i,c as m};
//# sourceMappingURL=footer-Caa3Bft0.js.map
