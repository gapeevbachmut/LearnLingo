import{c as p,b as i,i as v,g,s as w,d as E,u as k,o as S}from"./vendor-DEkrjftq.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))s(e);new MutationObserver(e=>{for(const o of e)if(o.type==="childList")for(const n of o.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&s(n)}).observe(document,{childList:!0,subtree:!0});function a(e){const o={};return e.integrity&&(o.integrity=e.integrity),e.referrerPolicy&&(o.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?o.credentials="include":e.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function s(e){if(e.ep)return;e.ep=!0;const o=a(e);fetch(e.href,o)}})();const M=`<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style="position:absolute;width:0;height:0;overflow:hidden">
  <defs>
...
`;function H(){const r=document.createElement("svg");r.style.display="none",r.innerHTML=M,document.body.prepend(r)}const L=p({username:i().required("Name is required").min(3,"Minimum 3 characters").max(30,"Max 30 characters"),email:i().required("Email is required").email("Invalid email"),phoneNumber:i().required("Phone is required").matches(/^\+?\d{10,14}$/,"Invalid phone number. +3801111111")}),x=p({email:i().required("Email is required").email("Invalid email"),password:i().required("Password is required").min(8,"Minimum 8 characters")}),A=p({username:i().required("Name is required").min(3,"Minimum 3 characters").max(30,"Max 30 characters"),email:i().required("Email is required").email("Invalid email"),password:i().required("Password is required").min(8,"Minimum 8 characters")});function y(r){const t={};r.inner.forEach(a=>{t[a.path]=a.message}),Object.entries(t).forEach(([a,s])=>{const e=document.querySelector(`[name="${a}"]`);if(!e)return;let o=e.nextElementSibling;(!o||!o.classList.contains("error"))&&(o=document.createElement("p"),o.classList.add("error"),e.after(o)),o.textContent=s})}function q(r){const t=document.querySelector(".form-lesson");t&&t.addEventListener("submit",async a=>{a.preventDefault(),t.querySelectorAll(".error").forEach(o=>o.remove());const s=new FormData(t),e=Object.fromEntries(s.entries());try{await L.validate(e,{abortEarly:!1}),console.log("BOOKING-FORM DATA:",e),r()}catch(o){y(o)}})}var l={};const I={apiKey:l.FIREBASE_API_KEY,authDomain:l.FIREBASE_AUTH_DOMAIN,projectId:l.FIREBASE_PROJECT_ID,storageBucket:l.FIREBASE_STORAGE_BUSKET,messagingSenderId:l.FIREBASE_MESSAGING_SENDER_ID,appId:l.FIREBASE_APP_ID},B=v(I),b=g(B);async function C(r,t){return(await w(b,r,t)).user}function D(r){const t=document.querySelector(".form-login");t&&t.addEventListener("submit",async a=>{a.preventDefault(),t.querySelectorAll(".error").forEach(o=>o.remove());const s=new FormData(t),e=Object.fromEntries(s.entries());try{await x.validate(e,{abortEarly:!1}),console.log("LOGIN-FORM DATA:",e),await C(e.email,e.password),r()}catch(o){y(o)}})}function z(){return`
...
<button class="btn-register" type='submit'>Sign Up</button>
  ...
  `}async function P(r,t,a){const s=await E(b,t,a);return await k(s.user,{displayName:r}),s.user}function R(r){const t=document.querySelector(".form-register");t&&t.addEventListener("submit",async a=>{a.preventDefault(),t.querySelectorAll(".error").forEach(o=>o.remove());const s=new FormData(t),e=Object.fromEntries(s.entries());try{await A.validate(e,{abortEarly:!1}),console.log("REGISTER-FORM DATA:",e),await P(e.username,e.email,e.password),r()}catch(o){y(o)}})}const c=document.querySelector("#modal-root");function _(){document.body.classList.add("no-scroll")}function N(){document.body.classList.remove("no-scroll")}function d(r){const{type:t,teacher:a}=r;let s="";switch(t){case"trial":s=F(a);break;case"login":s=z();break;case"register":s=O();break;default:return}c.innerHTML=s,_();const e=c.querySelector(".backdrop"),o=c.querySelector(".closeButton"),n=()=>{c.innerHTML="",N(),document.removeEventListener("keydown",h)},h=f=>{f.key==="Escape"&&n()};e.addEventListener("click",f=>{f.target===e&&n()}),o.addEventListener("click",n),document.addEventListener("keydown",h),t==="trial"&&q(n),t==="login"&&D(n),t==="register"&&R(n)}const u=document.querySelector(".logIn"),m=document.querySelector(".register");u&&u.addEventListener("click",r=>{r.preventDefault(),d({type:"login"})});m&&m.addEventListener("click",r=>{r.preventDefault(),d({type:"register"})});S(b,r=>{r?(console.log("name",r.displayName),console.log("Logged in:",r.email)):(u&&u.addEventListener("click",t=>{t.preventDefault(),d({type:"login"})}),m&&m.addEventListener("click",t=>{t.preventDefault(),d({type:"register"})}),console.log("Not logged in"))});const T=new Date;document.querySelector(".footer-date");const j=document.querySelector(".date-year");j.textContent=T.getFullYear();export{H as i,d as m};
//# sourceMappingURL=footer-D93N8gZN.js.map
