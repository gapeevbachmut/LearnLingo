...
`;function k(){const e=document.createElement("svg");e.style.display="none",e.innerHTML=S,document.body.prepend(e)}function x(e){return`
...
${e.levels.map(t=>`<li class='teacher-level'><p>#${t}</p></li>`).join("")}
    ...
    `}const l=document.querySelector(".teachers-list"),y=document.querySelector(".load-more-btn"),$=4;let m=1,i=[];function c(e){i=e,m=1,b()}function b(){const e=m*$,t=i.slice(0,e),a=t.map(n=>M(n)).join("");l.innerHTML=a,t.length>=i.length&&(y.style.display="none")}y.addEventListener("click",()=>{m+=1,b()});l.addEventListener("click",e=>{const t=e.target.closest(".teacher-read-more");if(!t)return;const a=t.closest(".teacher-card"),n=a.querySelector(".teacher-extra"),s=a.querySelector(".trial-lesson-btn");l.querySelectorAll(".teacher-card").forEach(r=>{r!==a&&(r.querySelector(".teacher-extra")?.classList.add("is-hidden"),r.querySelector(".trial-lesson-btn")?.classList.add("is-hidden"),r.querySelector(".teacher-read-more")?.classList.remove("is-hidden"))}),n.classList.toggle("is-hidden")?(t.classList.remove("is-hidden"),s.classList.add("is-hidden")):(t.classList.add("is-hidden"),s.classList.remove("is-hidden"))});const d=document.querySelector(".scroll-top"),w=()=>{window.scrollY>350?d.classList.remove("is-hidden"):d.classList.add("is-hidden")};window.addEventListener("scroll",w);w();const q=()=>window.scrollTo({top:0,behavior:"smooth"});d.addEventListener("click",q);const u=document.querySelector(".language");let p=[];function E(e){p=e;const t=e.flatMap(n=>n.languages);[...new Set(t)].forEach(n=>{const s=document.createElement("option");s.value=n,s.textContent=n,u.append(s)}),u.value=""}u.addEventListener("change",e=>{const t=e.target.value;if(!t){c(p);return}const a=p.filter(n=>n.languages.includes(t));c(a)});const T=new Date;document.querySelector(".footer-date");const C=document.querySelector(".date-year");C.textContent=T.getFullYear();const z="https://learnlingo-12-25-default-rtdb.firebaseio.com/.json";async function B(){const e=await L.get(z);return e.data?Object.values(e.data):[]}const h=document.querySelector(".level");let f=[];function P(e){f=e;const t=e.flatMap(n=>n.levels);[...new Set(t)].forEach(n=>{const s=document.createElement("option");s.value=n,s.textContent=n,h.append(s)}),h.value=""}h.addEventListener("change",e=>{const t=e.target.value;if(!t){c(f);return}const a=f.filter(n=>n.levels.includes(t));c(a)});const v=document.querySelector(".price");let g=[];function A(e){g=e;const t=e.map(n=>n.price_per_hour);[...new Set(t)].sort((n,s)=>n-s).forEach(n=>{const s=document.createElement("option");s.value=n,s.textContent=`${n} $`,v.append(s)}),v.value=""}v.addEventListener("change",e=>{const t=e.target.value,a=Number(t);if(!t){c(g);return}const n=g.filter(s=>s.price_per_hour===a);c(n)});k();async function _(){const e=await B();c(e),E(e),P(e),A(e)}_();
//# sourceMappingURL=main-NJAq5Ojw.js.map
